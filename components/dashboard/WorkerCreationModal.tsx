...
export function WorkerCreationModal({ isOpen, onClose, onSuccess }: WorkerCreationModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Call onSuccess callback to refresh worker list
        if (onSuccess) {
          onSuccess()
        }
      } else {
        ...
        if (response.status === 401) {
          ...
          setError('Access denied. You need admin privileges to create workers.')
        } else if (data.errors) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={success ? 'Worker Created Successfully!' : 'Create New Worker Login'}
    >
      {!success ? (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded text-sm text-blue-800 dark:text-blue-200">
            ...
            <ul className="list-disc list-inside space-y-1 text-xs">
              ...
              <li>Worker can reset password using forgot password option</li>
            </ul>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 p-4 rounded">
            <p className="text-sm text-green-800 dark:text-green-200 mb-2">
              Worker account created successfully! SMS sent to {generatedCredentials?.phone}
            </p>
          </div>
